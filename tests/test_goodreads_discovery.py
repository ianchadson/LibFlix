...
class ReviewSurfaceTemplateTests(unittest.TestCase):
    def test_shared_cards_and_dynamic_surfaces_carry_rating_data(self):
        ...
        self.assertIn("reviewUrl.hash = 'reviewsSection'", navbar)
        self.assertIn("badge.dataset.reviewJump = 'true'", navbar)
        self.assertIn("section.scrollIntoView({ block: 'start' })", book_page)
